...
export const NOCSearch = {
  ...
  applicationDetails: async (t, tenantId, applicationNumber, userType) => {
    ...
    const buildingDetails = {
      ...
      values: [
        { title: "NOC_APP_NO_LABEL", value: <div><Link to={`/digit-ui/employee/obps/bpa/${bpaResponse?.[0]?.applicationNo}`}><span className="link" style={{color: "#23316b"}}>{bpaResponse?.[0]?.applicationNo}</span></Link></div> },
