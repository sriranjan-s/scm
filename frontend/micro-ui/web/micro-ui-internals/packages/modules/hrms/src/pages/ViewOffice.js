...
const ViewOffice = () => {
    const [offices] = useState(window.Digit.SessionStorage.get("initData").offices);
    const history = useHistory();

    const handleModify = (department) => {
        history.push({
            pathname: '/digit-ui/employee/hrms/EditOffice',
            state: { department }, // Passing selected department data
        });
    };
console.log("officesoffices",offices)
    return (
        <div>
            <style>
                {`
